...
3. Footer follow-up: test the grouped selected-step controls at minimum DAW size and decide whether globals should collapse behind a compact utility strip.
...
- Current performance pass: StepGrid now caches static ruler/lane/beat chrome, repaints only old/new playhead columns during playback, and runs chain animation timers only while chains exist.
