...
const stepScheduler = read("src/engine/StepScheduler.h");
...
assertContains(processBlock, "schedulerChunkCapacity", "oversized and offline host blocks must honor scheduler-safe chunk capacity");
assertContains(processBlock, "blockOffset += chunkSamples", "host blocks must advance by each bounded processing chunk");
...
assertContains(stepScheduler, "absoluteStepIndex", "scheduler segments must preserve signed absolute host steps");
assertContains(processorHeader, "TriggerIdentity", "processor must retain transport-aware onset identities");
assertContains(processorHeader, "transportGeneration", "processor must distinguish host loop, seek, and restart epochs");
assertContains(processorHeader, "std::atomic<bool> triggerIdentityInvalidationRequested", "state restore must request onset invalidation without racing the audio thread");
assertContains(processor, "triggerIdentityInvalidationRequested.exchange", "audio thread must consume state-restore onset invalidation atomically");
assertContains(stepScheduler, "getMaximumSafeBlockSize", "scheduler must bound fixed-stack batches before the segment cap");
assertContains(processBlock, "std::isfinite(*hostPpq)", "host sync must accept finite negative PPQ while rejecting invalid positions");
assertContains(processBlock, "if (!segment.playing)", "stopped host segments must not advance sequenced effects");
assertContains(processorTests, "continuous host blocks trigger once before a same-step sample-position loop", "processor tests must distinguish continuous host blocks from loops");
assertContains(processorTests, "PPQ loops retrigger even when host sample time stays monotonic", "processor tests must detect PPQ loops independently of sample time");
assertContains(processorTests, "stopped host blocks neither process nor consume onsets", "processor tests must freeze sequenced effects while host transport is stopped");
assertContains(processorTests, "negative host PPQ triggers the normalized absolute step", "processor tests must cover negative host preroll");
assertContains(processorTests, "transport epochs retrigger every onset-driven lane", "processor tests must cover Slice, Loop, and Filter transport onsets");
assertContains(processorTests, "large tempo changes cannot hide short PPQ loops", "processor tests must reject tempo tolerance that masks host loops");
assertContains(processorTests, "stopped host keeps output gain and bypass contracts", "processor tests must retain the stopped live-monitor output contract");
assertContains(processorTests, "state restore requests realtime-safe onset invalidation", "processor tests must cover state-restore onset invalidation");
assertContains(processorTests, "host offline blocks preserve every pattern-cycle onset", "processor tests must cover more boundaries than one fixed scheduler batch");
assertContains(processor, "sanitizeAudioSampleRate", "processor must normalize invalid sample rates before preparing engines");
assertContains(processorTests, "invalid host sample rates are normalized before engine prepare", "processor tests must cover invalid sample-rate normalization through pitch rendering");
